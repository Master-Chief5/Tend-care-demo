// screens-a-driving.jsx — Driving / mileage tracking

function ScreenA_Driving() {
  return (
    <div className="phone-screen">
      <div style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
        <div style={{ padding: '14px 22px 4px', display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between' }}>
          ...
          <button style={{ background: 'var(--a-ink)', color: 'var(--a-card)', border: 0, borderRadius: 999, padding: '8px 14px', fontSize: 12, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, fontFamily: 'Geist' }}>
            <IconPlus size={14} sw={2.4} /> Start trip
          </button>
        </div>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '14px 22px 24px' }}>
          ...
          <div style={{ background: 'var(--a-ink)', color: '#fbf6ec', borderRadius: 16, padding: '16px 18px', marginBottom: 14, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <div className="serif" style={{ fontSize: 22, letterSpacing: '-0.01em', lineHeight: 1.15 }}>M. Lee → Dr. Patel's office</div>
          ...
          <div style={{ background: 'var(--a-card)', border: '1px solid var(--a-line)', borderRadius: 14, overflow: 'hidden', marginBottom: 14 }}>
            <TripRow time="Today · 9:14a" person="Devon P." house="willow" from="Willow" to="Walmart" miles="3.8" purpose="Grocery" />
            <TripRow time="Today · 8:02a" person="Saira K." house="maple" from="Maple" to="Day program" miles="6.1" purpose="Program" />
            <TripRow time="Mon · 4:40p" person="Aisha M." house="oak" from="Oak" to="Dr. Patel" miles="4.2" purpose="Medical" />
            <TripRow time="Mon · 11:20a" person="Jay B." house="oak" from="Oak" to="Library" miles="1.6" purpose="Activity" last />
          </div>
          ...
          <div style={{ background: 'var(--a-card)', border: '1px solid var(--a-line)', borderRadius: 14, padding: '12px 14px' }}>
            ...
            <VehicleRow name="Van #2 · Sienna '21" sub="Oak / Willow · Aisha out · 51,108 mi" status="active" />
            <VehicleRow name="Van #3 · Odyssey '23" sub="Maple / Cedar · Oil due 4/8" status="due" last />
          </div>
        </div>
      </div>
      <TabBar active="drive" />
...
function TripRow({ time, person, house, from, to, miles, purpose, last }) {
  const h = HOUSES.find(x => x.id === house);
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 14px', borderBottom: last ? '' : '1px solid var(--a-line)' }}>
...
function VehicleRow({ name, sub, status, last }) {
  const m = {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderBottom: last ? '' : '1px dashed var(--a-line)' }}>
...
Object.assign(window, { ScreenA_Driving, TripRow, VehicleRow, Mini });
